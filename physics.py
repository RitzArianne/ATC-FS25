...
class quaternion() :
    ...
    def hamilton_product(q1 : 'quaternion', q2: 'quaternion') -> 'quaternion':
        """Quaternion multiplication (Hamilton product): q = q1 * q2"""
        w1, x1, y1, z1 = q1.value
...
class physics_object ():
    ...
    def update_quat_from_ang_vel(self):
        ...
        if w_norm != 0:
            ...
            self.W_q_B = self.W_q_B / np.linalg.norm(self.W_q_B.value)

    def physics_step(self, frame: frames = frames.B , forces : NDArray = np.zeros((1,3)), application_points : NDArray = np.zeros((1,3))):
        """
        in-args :
        ...
        if frame == frames.W:
            raise(NotImplementedError, f"did not yet implement world frame: {frames.W}")
